...
<Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent 
        className="bg-white text-gray-900 h-[90vh] md:h-auto overflow-hidden flex flex-col p-0"
        style={{ width: '95vw', maxWidth: '95vw', margin: '0 auto' }} // Forced inline width
      >
        <div className="flex-1 overflow-y-auto p-0" style={{ width: '100%', overflowX: 'hidden' }}>       
            ...
            <div className="relative" style={{ width: '100%' }}>
                ...
                <div 
                    ref={modalContentRef} 
                    className={`p-4 md:p-8 transition-all duration-700 ${!isUnlocked ? 'blur-2xl pointer-events-none opacity-50' : 'blur-0 opacity-100'}`}
                    style={{ width: '100%', maxWidth: '100%' }}
                >
                    <DialogHeader style={{ width: '100%' }}>
                        {/* Title: Forced to wrap and shrink size using inline style */}
                        <DialogTitle 
                            className="font-bold text-center text-blue-900 uppercase tracking-tight"
                            style={{ fontSize: '18px', whiteSpace: 'normal', wordBreak: 'break-word', display: 'block' }}
                        >
                    ...
                    <div className="my-6 border-4 border-dashed border-green-500 rounded-xl p-4 bg-green-50 text-center" style={{ width: '100%', boxSizing: 'border-box' }}>
                        {/* Stacking logic using flex-wrap to ensure it breaks on small screens */}
                        <div className="flex flex-wrap justify-center items-center gap-4">
                            ...
                            {/* Divider: Hidden on small screens via inline style check if possible, or just standard classes */}
                    ...
                    {/* Table: Forced to stay inside portrait frame via overflow */}
                    <div className="border rounded-xl overflow-hidden shadow-sm" style={{ width: '100%', overflowX: 'auto' }}>
                        <Table style={{ width: '100%', minWidth: '350px' }}>
                            <TableHeader className="bg-gray-50">
                                <TableRow>
                                    {headers.map((h, i) => <TableHead key={i} className="font-bold whitespace-nowrap" style={{ fontSize: '10px' }}>{h}</TableHead>)}
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {rows.map((row, ri) => (
                                    <TableRow key={ri}>
                                        {row.map((c, ci) => <TableCell key={ci} className="py-4 font-medium whitespace-nowrap" style={{ fontSize: '11px' }}>{c}</TableCell>)}
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
                ...
                {/* Overlay Layer */}
                {!isUnlocked && (
                    <div className="absolute inset-0 z-10 flex flex-col items-center justify-center p-6 bg-white/5" style={{ width: '100%' }}>
                        <div className="animate-in fade-in zoom-in duration-300 flex flex-col items-center bg-white p-6 md:p-10 rounded-3xl shadow-2xl border border-gray-100" style={{ maxWidth: '280px', width: '90%' }}>
                            ...
                            {!user ? (
                                <div className="flex flex-col items-center w-full">
                                    <h3 className="text-xl font-black mb-2 text-gray-900 text-center tracking-tight">Sign in</h3>
                                    <p className="text-gray-500 mb-6 text-[10px] text-center leading-relaxed">Sign in with Google to link this audit.</p>
                                ...
                                <div className="flex flex-col items-center w-full">
                                    <h3 className="text-xl font-black mb-2 text-gray-900 text-center tracking-tight">Locked Report</h3>
                                    <p className="text-gray-500 mb-6 text-[10px] text-center leading-relaxed">
                                        Unlock for $3.99/mon.<br />Accuracy is approx 80%.
                                    </p>
                                    <Button size="lg" className="bg-green-600 hover:bg-green-700 w-full font-extrabold py-6 text-lg shadow-xl" onClick={handleCheckout} disabled={isCheckingOut}>
        ...
        <DialogFooter className="px-4 pb-8 pt-4 flex flex-col gap-2">
            ...
            <DialogClose asChild>
                <Button variant="outline" className="w-full text-xs font-semibold">Close</Button>
            </DialogClose>
        </DialogFooter>
      </DialogContent>
    </Dialog>
